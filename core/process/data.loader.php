...
if(isset($lang)){
	...
	//$pokedex = SYS_PATH.'/core/json/pokelist_FR.json'; 
...
if(!empty($page)){
	...
	switch($page){
		...
		case 'trainers':
			...
			$req = "SELECT name, level FROM trainer ORDER BY level DESC LIMIT 100";
			if($trainer_name != ""){
				$req = "SELECT name, level FROM trainer WHERE name LIKE '%".$trainer_name."%' ORDER BY level DESC LIMIT 100";
			}
            $result = $mysqli->query($req);
	        $trainers = array();
	        while($data = $result->fetch_object()){
			...
			foreach($trainers as $trainer){
				$reqPkms = "SELECT DISTINCT * FROM `gympokemon` WHERE trainer_name LIKE '".$trainer->name."' ORDER BY cp DESC";
...
///////////// 
	...
	$reqPkmsR = "SELECT DISTINCT pokemon_id, encounter_id, latitude, longitude, disappear_time FROM pokemon ".$filterlist." ORDER BY disappear_time DESC LIMIT 0,12";
