...
switch($request){
	...
	$filterlist = $config->system->filterpoke;
	$filterlist = "WHERE pokemon_id != '".str_replace(",", "' AND pokemon_id != '", $filterlist)."'";
	
	$reqPkmsR = "SELECT DISTINCT pokemon_id, encounter_id, disappear_time, latitude, longitude FROM pokemon ".$filterlist." ORDER BY disappear_time DESC LIMIT 0,12";

	$resultPkmsR = $mysqli->query($reqPkmsR);
				$recents = array();
				$i=0;
				while($dataPkmR = $resultPkmsR->fetch_object()){
					$recents[$i++] = $dataPkmR;
				}
	...
	case 'spawnlist_update':
		...
		$req 			= "SELECT pokemon_id,  disappear_time, latitude, longitude FROM pokemon $filterlist ORDER BY disappear_time DESC LIMIT 0,1";
		$result 			= $mysqli->query($req);
		$recents			= array();
		$data 				= $result->fetch_object();
		$pokeid 			= $data->pokemon_id;
		$pdisappear_time 	= $data->disappear_time;
		$platitude			= $data->latitude;
		$plongitude			= $data->longitude;
		
		$pokelist_file		= SYS_PATH.'/core/json/pokelist_FR.json'; 
		$pokemon_file 		= file_get_contents($pokelist_file); 
		$pokemons			= json_decode($pokemon_file);
		
		$date = new DateTime($data->disappear_time);
		...
		if($_GET['last_id'] != $pokeid){
			...
			<div class="col-md-1 col-xs-4 wow " pokeid="'.$pokeid.'" style="display:none;">
				...
				<a href="http://maps.google.fr/maps?f=q&hl=fr&q='.$platitude.','.$plongitude.'"><img src="core/pokemons/'.$pokeid.'.png" class="img-responsive"></a>
				<p class="pkmn-name"><a href="http://maps.google.fr/maps?f=q&hl=fr&q='.$platitude.','.$plongitude.'">'.$pokemons->$pokeid->name.'<br/><small>disparaît à '.$timeleft.'</small></a></p>
