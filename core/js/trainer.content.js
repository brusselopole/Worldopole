...
function printPokemon(pokemon, pokeimg_suffix, iv_numbers, locale) {
	...
	if (pokemon.deployment_time) {
		...
		if (diff >= 86400) {
			trainerPokemon.append($('<small>', { text: parseInt(diff / 86400) + 'd ' + parseInt((diff / 3600) % 3600) + 'h'}));
		} else {
			trainerPokemon.append($('<small>', { text: parseInt(diff / 3600) + 'h ' + parseInt((diff / 3600) % 60) + 'm' }));
		}
