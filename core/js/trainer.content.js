		...
		$.ajax({
			...
			$.each(data, function(trainerName, trainer){
				...
				for(pokeIndex = 0; pokeIndex<trainer.pokemons.length;pokeIndex++){
					...
					progressBar.append($('<div>',{title: 'IV Stamina :'+pokemon.iv_defense, class: 'progress-bar progress-bar-info' ,role : 'progressbar', 'aria-valuenow': pokemon.iv_defense, 'aria-valuemin' : 0, 'aria-valuemax' : 45}).css('width',((100/45)*pokemon.iv_defense ) + '%' ))
